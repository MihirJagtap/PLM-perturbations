...
detect_perturbations1 <- function(residuals) {
  perturbations <- list()
  in_perturbation <- FALSE
  start <- end <- NA
  peaks_within_perturbation <- list()
    ...
    if (!is.na(residuals[i])) {
      if (residuals[i] < 0) {
        if (!in_perturbation) {
          in_perturbation <- TRUE
          start <- i
          peaks_within_perturbation <- NULL
        }
        # Detecting a local maximum in the downtrend
        if (residuals[i] > residuals[i-1] && residuals[i] > residuals[i+1]) {
          peaks_within_perturbation <- c(peaks_within_perturbation, i)
        }
        end <- i
      } else {
        if (in_perturbation) {
          # If we exit the perturbation, save the perturbation and any peaks within it
          if ((end - start) >= 5) {
            perturbations <- c(perturbations, list(list("start" = start, "end" = end, "peaks" = peaks_within_perturbation)))
          }
          in_perturbation <- FALSE
          peaks_within_perturbation <- NULL
        }
...
# unperturbed_yield <- zoo::rollapply(daily_milk_yield, width = 15, FUN = mean, fill = NA, align = "center")
residuals <- daily_milk_yield - predicted_milk_yields
perturbations <- detect_perturbations1(residuals)
#characterization <- characterize_perturbations(mv_data$MY_obs, perturbations)
...
axis(1)
axis(2)
#lines(TIME, MODELS[[i]][["predict"]], col="brown", lwd=2)
lines(Cow.data$time, wood(TIME, PARAM[i, "a"], PARAM[i, "b"], PARAM[i, "c"]), col="black", lwd=2)
legend("topright", box.lty=0, legend=c("data", "Unperturbed", "Perturbed"), cex=0.8, lty=c(NA, 1, 1), pch=c(21, NA, NA), lwd=c(1, 1.5, 1.5), col=c("blue", "black", "red"))

# Loop through intervals to add lines
for (interval in perturbations) {
    abline(v = interval[1], col = "red", lwd = 2, lty = 2)   # Start of the interval
    abline(v = interval[2], col = "green", lwd = 2, lty = 2) # End of the interval
}
```
